...
public class SimpleErrorResultHandler implements ErrorResultHandler {
    ...
    protected MediaType contentType2MediaType(String contentType){
        ...
        throw new IllegalArgumentException("unsupport contentType in this error handler...");
    }
    ...
    private ModelAndView produceHtml(Object errorResult,HttpStatus status) {
        ...
        if(errorResult instanceof ErrorResult){
            return new ModelAndView("redirect:http://ns.huatu.com/pc/error/5xx?code="+((ErrorResult) errorResult).getCode()+"&message="+((ErrorResult) errorResult).getMessage());
        }
        return new ModelAndView("redirect:http://ns.huatu.com/pc/error/5xx");
    }
