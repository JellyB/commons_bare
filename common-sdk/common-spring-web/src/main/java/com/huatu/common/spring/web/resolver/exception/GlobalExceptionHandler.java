...
public class GlobalExceptionHandler {
    ...
    @ResponseBody
    public void unauthorizedHandler(HttpServletRequest request, HandlerMethod handlerMethod, Exception exception) {
        ErrorResult errorResult = buildByResolvers(exception, ((UnauthorizedException)exception).getErrorResult());
        errorHandler.handle(request,handlerMethod,errorResult, HttpStatus.UNAUTHORIZED);
    }
    ...
    @ResponseBody
    public void bizErrorHandler(HttpServletRequest request, HandlerMethod handlerMethod, Exception exception) {
        ErrorResult errorResult = buildByResolvers(exception, ((BizException)exception).getErrorResult());
        errorHandler.handle(request,handlerMethod,errorResult, null);
    }
    ...
    public void invalidArgumentsHandler(HttpServletRequest request, HandlerMethod handlerMethod, Exception exception) {
        ErrorResult errorResult = buildByResolvers(exception, CommonErrors.INVALID_ARGUMENTS);
        errorHandler.handle(request,handlerMethod,errorResult, HttpStatus.BAD_REQUEST);
    }
    ...
    public void notFoundHandler(HttpServletRequest request,HandlerMethod handlerMethod,Exception exception){
        ErrorResult errorResult = buildByResolvers(exception, CommonErrors.RESOURCE_NOT_FOUND);
        errorHandler.handle(request,handlerMethod,errorResult, HttpStatus.NOT_FOUND);
    }
    ...
    public void globalErrorHandler(HttpServletRequest request, HandlerMethod handlerMethod, Exception exception){
        //不认识的错误，打印完整的异常信息
        log.error("catch exception : ",ExceptionUtils.getRootCauseStackTrace(exception));
